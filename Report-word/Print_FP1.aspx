...
<script runat="server">
    ...
    protected void WordOut() {
        string docFileName = "[質權]-" + ipoRpt.Seq + ".docx";

        Dictionary<string, string> _tplFile = new Dictionary<string, string>(){
			{"apply", Server.MapPath("~/ReportTemplate/申請書/紙本/FP1_設定質權登記申請書.docx")},
		};
        ...
        if (dmt.Rows.Count > 0) {
            ...
            if (dtTran.Rows.Count > 0) {
                if (dtTran.Rows[0].SafeRead("debit_money", "") != "") {
                    ipoRpt.ReplaceBookmark("debit_money1", "Ｖ");
                    ipoRpt.ReplaceBookmark("debit_money", Convert.ToInt32(dtTran.Rows[0].SafeRead("debit_money", "0")).ToString("N0"));
                }

                if (dtTran.Rows[0].SafeRead("other_item1", "") != "") {
                    ipoRpt.ReplaceBookmark("other_item1", "Ｖ");
                    string[] other_item = dtTran.Rows[0].SafeRead("other_item1", "").Split(';');
                    if (other_item.Length > 0 && other_item[0] != "") {
                        ipoRpt.ReplaceBookmark("other_item2", other_item[0]);
                    }
                    if (other_item.Length > 1 && other_item[1] != "") {
                        ipoRpt.ReplaceBookmark("other_item3", other_item[1]);
                    }
                }
            }
            //具結
