...
<script runat="server">
    ...
    private void Page_Load(System.Object sender, System.EventArgs e) {
        ...
        html_toadd = dtCase.Option("{Cust_code}", "{Code_name}", " v1='{form_name}'", false, "", "mark=S");
...
<input type=text name=job_case id=job_case value="<=job_case%>">
<TABLE id=tab15 style="display:" border=0 class="bluetable"  cellspacing=1 cellpadding=2 width="100%">
	...
	<TR >
		...
		<TD class=whitetablebg colspan=5 ><input type=text name="job_no" id="job_no" value="<=job_no%>">
			<input type=radio name="job_type" value="case" class="<%#Lock.TryGet("brt15Lock")%>" onclick="brt15form.showtr()">接洽客戶後續案性
			<input type=radio name="job_type" value="endr" class="<%#Lock.TryGet("brt15Lock")%>" onclick="brt15form.showtr()">結辦歸檔
		</td>
	</TR>
	...
	<TR >
		...
		<TD class=whitetablebg colspan=5>
			<input type=text name="pre_date" id="pre_date" size=10 value="<=pre_date%>" class="dateField <%#Lock.TryGet("brt15Lock")%>">
		</td>
	</TR>
	<TR >
		<TD class=lightbluetable align=right >交辦事項說明：</TD>
		<TD class=whitetablebg colspan=5>
			<textarea cols=60 rows=5 name=sales_remark id=sales_remark class="<%#Lock.TryGet("brt15Lock")%>"><=sales_remark%></textarea>
		</td>
	</TR>
	<TR>
	...
	<TR >
		...
		<TD class=whitetablebg colspan=5><input type=text name="ocs_report" id="ocs_report" value="<=cs_report%>">
			<input type=radio name="cs_report" value="Y" class="<%#Lock.TryGet("brt15Lock")%>" onclick="brt15form.cs_report_onclick1(this.value)" >需
			<input type=radio name="cs_report" value="N" class="<%#Lock.TryGet("brt15Lock")%>" onclick="brt15form.cs_report_onclick1(this.value)" >不需
		</td>
	</TR>
	<TR>
