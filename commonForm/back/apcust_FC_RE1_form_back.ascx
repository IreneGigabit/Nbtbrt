<%@ Control Language="C#" ClassName="apcust_FC_RE1_form" %>
...
<table border="0" id=tabap class="bluetable" cellspacing="1" cellpadding="2" width="100%">
	    ...
	    <TR>
		    <TD class="bluetable sfont9" style="border-color:red" colspan=4 align=right>
                <span id="span_btn_POA">
    			    <input type=button class="greenbutton <%#Lock.TryGet("Qdisabled")%>" value="查詢申請人委任書" onclick="apcust_form.btn_POA('tabap','dbmn1_')">
			    </span>
	...
	<TBODY></TBODY>
    <script type="text/html" id="apcust_fc_re1_template_0">
	    <TR>
		    <TD class=lightbluetable align=right>
			    <input type=text id="apnum_##" name="apnum_##" value="##." class="Lock" size=2>
		    </TD>
		    <TD class=whitetablebg>
		        ...
		        <input type='checkbox' id='ap_hserver_flag_##' name='ap_hserver_flag_##' value='Y' onclick="apcust_form.apserver_flag('##','')">此申請人為選定代表人
	    ...
	    <TR>
		    ...
		    <TD class=whitetablebg title="輸入編號並點選確定，即顯示申請人資料；若無資料，請直接輸入申請人資料。">
		        ...
		        <input TYPE=text id='dbmn1_new_no_##' NAME='dbmn1_new_no_##' SIZE=11 MAXLENGTH=10 onblur="apcust_form.chkapcust_no(reg.apnum.value,'##','dbmn1_new_no_')">&nbsp;
	    ...
	    <TR>
		    ...
		    <TD class=whitetablebg title="輸入關鍵字並點選申請人查詢，即顯示申請人資料清單。">
		        申請人名稱(中)：
		        <INPUT TYPE=text id=dbmn1_ncname1_## NAME=dbmn1_ncname1_## SIZE=40 alt='『申請人名稱(中)』' MAXLENGTH=60 onblur="fDataLen(this)">
		        <INPUT TYPE=text id=dbmn1_ncname2_## NAME=dbmn1_ncname2_## SIZE=40 alt='『申請人名稱(中)』' MAXLENGTH=60 onblur="fDataLen(this)">
	    ...
	    <TR>
		    <TD class=lightbluetable align=right></TD>
		    <TD class=whitetablebg>
		        <input type=button class='cbutton <%#Lock.TryGet("QA1disabled")%>' value='查詢' onclick="apcust_form.get_apnameaddr('##', '', 'dbmn1_')">申請人名稱(英)：
...
<table border="0" id=FC1_tabap class="bluetable" cellspacing="1" cellpadding="2" style="font-size: 9pt" width="100%">
	<tr id="old_no_head">
		<td class="lightbluetable" colspan="4" valign="top" STYLE="cursor:pointer;COLOR:BLUE" onclick="PMARK(c1Oapcust)"><strong>伍、<u>原申請人(如不是申請權利之讓與，可免填本項)</u></strong></td>
	</tr>
	<TR>
...
<script language="javascript" type="text/javascript">
    ...
    $("#AP_Add_button").click(function () {
        ...
        //複製樣板
        var copyStr = $("#apcust_fc_re1_template_0").text() || "";
        copyStr = copyStr.replace(/##/g, nRow);

        $("#tabap>tbody").append("<tr id='tr_ap_fc_re1_0_" + nRow + "' class='sfont9'><td><table border='0' class='bluetable' cellspacing='1' cellpadding='2' width='100%'>" + copyStr + "</table></td></tr>");
        $("#tr_ap_fc_re1_0_" + nRow + " .Lock").lock();
        $("#apnum").val(nRow);
    });
    ...
    $("#FC1_AP_Add_button").click(function () {
        ...
        $("#tabap>tbody").append("<tr id='tr_ap_fc_re1_1_" + nRow + "' class='sfont9'><td><table border='0' class='bluetable' cellspacing='1' cellpadding='2' width='100%'>" + copyStr + "</table></td></tr>");
        $("#tr_ap_fc_re1_1_" + nRow + " .Lock").lock();
        $("#FC1_apnum").val(nRow);
    });
    ...
    apcust_form.getappx1 = function (nRow) {
        $.ajax({
            ...
            error: function (xhr) {
                ...
                $("#dialog").dialog({ title: '原申請人資料載入失敗！', modal: true, maxHeight: 500, width: 800 });
                //toastr.error("<a href='" + this.url + "' target='_new'>原申請人資料載入失敗！<BR><b><u>(點此顯示詳細訊息)</u></b></a>");
            }
        });
    }
