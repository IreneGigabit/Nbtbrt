				...
				try {
					...
					using (SqlDataReader dr = conn.ExecuteReader(SQL)) {
						if (!dr.Read()) {
                            ...
                            string[] arr_appl = dr.SafeRead("form_name", "").Split('|');//案件名稱tag|變更後案件名稱tag
                            string appl_line = Get_name(arr_appl[0]);//案件名稱tag
							...
							using (SqlDataReader dr1 = conn.ExecuteReader(SQL)) {
								if (!dr1.Read()) {
                                    ...
                                    string appl_name = dr1.SafeRead("appl_name", "").Trim();
                                    dr1.Close();
                                    if (appl_name.ToXmlUnicode() != split_appl[1].Trim().ToXmlUnicode()) {
                                        SQL = "select ncname1,a.rs_code " +
                                                "from attcase_dmt a " +
                                                "join dmt_tranlist t on a.in_no=t.in_no and t.mod_field = 'mod_dmt' " +
                                                "where att_sqlno='" + Request["att_sqlno"] + "' " +
                                                "and seq = '" + Request["seq"] + "' and seq1 = '" + Request["seq1"] + "' ";
                                        using (SqlDataReader dr2 = conn.ExecuteReader(SQL)) {
                                            if (!dr2.Read()) {
                                                if (dr2.SafeRead("rs_code", "") == "FR1") {//延展案若有變更商標名稱,則用變更後的名稱比對

                                                } else {
                                                    Response.Write("	errFlag=true;\r\n");
                                                    Response.Write("	$('#msg').append('<Font align=left color=\"red\" size=3>" + split_appl[0] + "】申請書案件名稱(" + split_appl[1].Trim() + ")與案件主檔(" + appl_name + ")不符!!</font><BR>');\r\n");
                                                }
                                            } else {
                                                Response.Write("	errFlag=true;\r\n");
                                                Response.Write("	$('#msg').append('<Font align=left color=\"red\" size=3>" + split_appl[0] + "】申請書案件名稱(" + split_appl[1].Trim() + ")與案件主檔(" + appl_name + ")不符!!</font><BR>');\r\n");
