...
<script runat="server">
    ...
    DBHelper connOptK = null;//開完要在Page_Unload釋放,否則sql server連線會一直佔用
    private void Page_Unload(System.Object sender, System.EventArgs e) {
        if (conn != null) conn.Dispose();
        if (connOptK != null) connOptK.Dispose();
    }
    ...
    private void doUpdateDB() {
        ...
        connOptK.ExecuteNonQuery(SQL);

        //抓insert後的流水號
        SQL = "SELECT SCOPE_IDENTITY() AS Current_Identity";
        string opt_sqlno = (connOptK.ExecuteScalar(SQL) ?? "").ToString();
    ...
    private void CreateMail(string case_no) {
        ...
        using (SqlDataReader dr = connOptK.ExecuteReader(SQL)) {
            if (dr.Read()) {
