...
<script runat="server">
    ...
    protected void WordOut() {
        ...
        using (DBHelper conn = new DBHelper(Conn.btbrt).Debug(false)) {
            SQL = "select a.*,b.attach_name,(select sc_name from sysctrl.dbo.scode where scode=a.dmt_scode) as sc_name,";
            ...
            SQL += "inner join dmt_attach b on a.seq=b.seq and a.seq1=b.seq1 and a.step_grade=b.step_grade ";
            SQL += "where b.source='scan' and b.attach_flag<>'D' ";
            if ((Request["qrydateS"] ?? "") != "") SQL += " and b.in_date>='" + Request["qrydateS"] + " 00:00:00'";
            if ((Request["qrydateE"] ?? "") != "") SQL += " and b.in_date<='" + Request["qrydateE"] + " 23:59:59'";
            if ((Request["qrybseq"] ?? "") != "") SQL += " and b.seq>='" + Request["qrybseq"] + "'";
            if ((Request["qryeseq"] ?? "") != "") SQL += " and b.seq<='" + Request["qryeseq"] + "'";
            if ((Request["qryseq1"] ?? "") != "") SQL += " and b.seq1='" + Request["qryseq1"] + "'";
            if ((Request["hchk_status"] ?? "") != "") SQL += " and b.chk_status like '" + Request["hchk_status"] + "%'";
            SQL += " order by " + Request["qryOrder"];
            ...
            if (dt.Rows.Count > 0) {
                ...
                for (int i = 0; i < dt.Rows.Count; i++) {
                    ...
                    if (dt.Rows[i].SafeRead("attach_name", "") != "") {
                        string attach_name = dt.Rows[i].SafeRead("attach_name", "");
                        tbarcode = "*SIIPLO-" + Path.GetFileNameWithoutExtension(attach_name) + "*";
                    } else {
                        tbarcode = "*SIIPLO-" + dt.Rows[i].SafeRead("branch", "") + Sys.GetSession("dept").ToUpper() + "-" + dt.Rows[i].SafeRead("seq", "").PadLeft(Sys.DmtSeq, '0') + "-" + tseq1 + "-" + dt.Rows[i].SafeRead("step_grade", "").PadLeft(4, '0') + "*";
                    }
