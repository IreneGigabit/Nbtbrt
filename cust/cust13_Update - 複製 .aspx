...
<script runat="server">
    ...
    private void ProcessAdd()
    {
        ...
        SqlDataReader drCustz = conn.ExecuteReader("SELECT cust_area, cust_seq, id_no FROM custz WHERE id_no = '" + ReqVal.TryGet("apcust_no") + "'");
        if (drCustz.Read())
        {
            CustArea = "N"; CustSeq = int.Parse(drCustz["cust_seq"].ToString());
        }
    ...
    /*
     * //****不需要.已有Util.dbnull / 已有Util.dbchar / 已有Util.dbzero
...
<script type="text/javascript" language="javascript">
    ...
    window.location.href = "cust13_List.aspx?prgid=" + $("#prgid").val() + "&apcust_no=<%=GetApcustNO%>&submitTask=U";
    alert("<%=msg%>");
