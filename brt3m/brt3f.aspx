    ...
    private void PageLayout() {
        ...
        using (DBHelper conn = new DBHelper(Conn.btbrt).Debug(false)) {
            ...
            if ((HTProgRight & 256) != 0) {
                SQL = "select a.job_scode,b.sc_name from todo_dmt a ";
                SQL += "inner join sysctrl.dbo.scode b on a.job_scode=b.scode ";
                SQL += "where a.job_status='NN' and (a.dowhat='DB_END') and a.syscode='" + Session["syscode"] + "' ";
                SQL += "group by a.job_scode,b.sc_name";
            } else {
                SQL = "select A.job_scode,B.grplevel,B.grpid,d.sc_name from todo_dmt A ";
                SQL += "left join sysctrl.dbo.scode_group C on a.job_scode = c.scode and c.GrpClass=a.branch ";
                SQL += "left join sysctrl.dbo.grpid B on c.grpclass=b.grpclass and c.grpid=b.grpid and (substring(b.grpid,1,1)='T' or substring(b.grpid,1,3)='000' or substring(b.grpid,1,1)='A') ";
            ...
            if (dt.Rows.Count > 0) {
                ...
                //營洽清單
                SQL = "select A.case_IN_scode,d.sc_name from todo_dmt A ";
                SQL += "inner join sysctrl.dbo.scode D on a.case_in_scode = d.scode ";
                SQL += "where a.job_status='NN' and a.dowhat='DB_END' and a.syscode='" + Session["syscode"] + "' ";
                SQL += "group by a.case_in_scode,d.sc_name";
                DataTable dtscode = new DataTable();
                conn.DataTable(SQL, dtscode);
                td_tscode = "<select id='from_scode' name='from_scode'>" + dtscode.Option("{case_IN_scode}", "{sc_name}") + "</select>";
            } else {
                //營洽清單
                td_tscode = "<select id='from_scode' name='from_scode'><option value='*'>全部</option></select>";
            }
        }
    }
</script>
        ...
        <table id="qryForm" border="0" class="bluetable" cellspacing="1" cellpadding="2" width="70%" align="center">	
            <tr>
		        ...
		        <td class="whitetablebg" align="left">
                    <select id='job_scode' name='job_scode' onchange="searchScode(this.value,'from_scode','ENDT')" >
                        <%#td_jscode%>
                    </select>
                </td>
	        </tr>
