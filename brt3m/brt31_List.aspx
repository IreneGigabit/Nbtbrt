...
<script runat="server">
    ...
    protected string txtSMaster = "", selManager = "",txt_agentNm="無",txt_agentNo="", selPrScode = "", selAccScode = "";
    ...
    private void QueryData() {
        ...
        using (DBHelper conn = new DBHelper(Conn.btbrt).Debug(false)) {
            if (qs_dept == "t") {
                ...
                SQL += ",''ctrl_rowspan,''tar_mark,''dis_flag,''disT_flag,''chk_stat,''accdchk_flag,''tran_remark1,''sign_level,''sign_levelnm ";
                SQL += "FROM Case_dmt B ";
			...
			<ItemTemplate>
 		        <tr class="<%#(Container.ItemIndex+1)%2== 1 ?"sfont9":"lightbluetable3"%>">
		            <td align="center" rowspan=<%#Eval("ctrl_rowspan")%>>
		                ...
		                <input type=hidden id="upload_chk_<%#(Container.ItemIndex+1)%>" name="upload_chk_<%#(Container.ItemIndex+1)%>" value="<%#Eval("upload_chk")%>"><!--請核單上傳-->
		                <input type=hidden id="ar_mark_<%#(Container.ItemIndex+1)%>" name="ar_mark_<%#(Container.ItemIndex+1)%>" value="<%#Eval("tar_mark")%>"><!--扣收入註記-->
...
<FooterTemplate>
    ...
    <table border="1" width="90%" cellspacing="0" cellpadding="0" align="center">
		<TR>			
			...
			<TD align=left>
					<input type=radio name="signid" value="YY" onclick=tosign()>簽准
					<input type=radio name="signid" value="XX" onclick=tosign()>不准退回
					<input type=radio name="signid" value="YT" onclick=tosign()>轉上級簽核
			...
			<TD class="whitetablebg" align=right>
				<span style="display:" id="showsign">
					<input type=radio name="upsign" value="sMaster"><%#txtSMaster%>
                    <input type=radio name="upsign" value="sManager"><select name="ma_scode" id="ma_scode"><%#selManager%></select>
                    <input type=radio name="upsign" value="sAgent">代理人:<%#txt_agentNm%><input type=hidden value="<%#txt_agentNo%>" name="sAgentcode" id="sAgentcode"><input type=hidden value="S" name=mark id=mark>	
				</span>
			</TD>
			<TD class="whitetablebg" align=right>
    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="0">
     <tr><td width="100%">     
       <p align="center">        
            <input type=button value ="送出" class="cbutton" onClick="formupdate()" id=btnsend name=btnsend>
...
<script language="javascript" type="text/javascript">
    ...
    function formupdate(){
        //判斷選項是簽准還是轉上級
        var sign_level =$("#sign_level").val().Left(1);//勾選案件執行的層級
        var grplevel =$("#grplevel").val();//簽核者的層級
        if (sign_level == "0") {//商標經理
        } else if (sign_level == "1") {//區所主管
            if (CInt(grplevel) > CInt(sign_level)) {
                alert("需經區所主管簽核，請點選「轉上級簽核」並選擇簽核主管！");
                $("input[name=signid][value='YT']").prop("checked", true).triggerHandler("click");
            }
        } else if (sign_level == "2") {//部門主管
            if (CInt(grplevel) > CInt(sign_level)) {
                alert("需經部門主管簽核，請點選「轉上級簽核」並選擇簽核主管！");
                $("input[name=signid][value='YT']").prop("checked", true).triggerHandler("click");
            }
        }
    }
    ...
    function tosign(){
        if ($("input[name=signid]:checked").val() == "YY") {//簽准
            ...
            if ($("#sign_level").val()=="11"){//需經會計檢核
                ...
                $("#showsign").show();//主管
                $("#showsign2").hide();//會計人員
                reg.upsign(0).checked = true
                if reg.upload_flag.value = "Y" then
                    if reg.grpid.value="000" then
                        reg.upsign(0).checked = true
                    else   
				        reg.upsign(1).checked = true
                    end if   
                end if
                if reg.grpid.value="T000x" then
                    reg.upsign(1).checked = true
                    end if
            }
        }
