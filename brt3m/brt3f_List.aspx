...
<script runat="server">
    ...
    private void PageLayout() {
        ...
        FormName = "備註:<br>\n";
        FormName += "※流程:程序結案案件處理-->會計確認--><font color=red>組主管簽核-->部門主管簽核-->區所主管簽核</font>-->程序確認結案暨掃描上傳<br>\n";
    ...
    private void QueryData() {
        ...
        SQL = " select t.apcode,a.cappl_name as appl_name,t.job_scode,t.sqlno as todo_sqlno,a.*";
        SQL += ",(select sc_name from sysctrl.dbo.scode where scode=a.dmt_scode) as sc_name ";
        ...
        SQL += " from todo_dmt t ";
        SQL += " inner join vstep_dmt a on  t.seq=a.seq and t.seq1=a.seq1 and t.step_grade=a.step_grade ";
        SQL += " where t.job_status='NN' and t.syscode='" + Session["syscode"] + "' and t.dowhat='DB_END' ";
        ...
        if (ReqVal.TryGet("from_scode") != "*" && ReqVal.TryGet("from_scode") != "") {
            SQL += " and a.scode1= '" + Request["from_scode"] + "'";
        }
    ...
    protected string GetBackIcon(RepeaterItem Container) {
        ...
        string from_flag = "", ap_desc = "";
        SQL = "select approve_desc from todo_dmt where seq=" + Eval("seq") + " and seq1='" + Eval("seq1") + "' and step_grade=" + Eval("step_grade") + " and from_flag='END' and dowhat='ACC_END' and job_status='NX' order by sqlno desc ";
        objResult = conn.ExecuteScalar(SQL);
        if (!(objResult == DBNull.Value || objResult == null)) {
            from_flag = "END_ACC";//會計退回
            ap_desc = objResult.ToString();
        }

        SQL = "select approve_desc from todo_dmt where seq=" + Eval("seq") + " and seq1='" + Eval("seq1") + "' and step_grade=" + Eval("step_grade") + " and from_flag='END' and dowhat='DB_END' and job_status='NX' order by sqlno desc ";
        objResult = conn.ExecuteScalar(SQL);
        if (!(objResult == DBNull.Value || objResult == null)) {
            from_flag = "END_DB";//主管退回
            ap_desc = objResult.ToString();
        }


        if (from_flag == "END_ACC") {
            rtn = "<img border=\"0\" src=\"" + Page.ResolveUrl("~/images/star_r.gif") + "\" title=\"會計退回原因：" + ap_desc + "\">";
        } else if (from_flag == "END_DB") {
            rtn = "<img border=\"0\" src=\"" + Page.ResolveUrl("~/images/star_pl.gif") + "\" title=\"主管退回原因：" + ap_desc + "\">";
        }
    ...
    <table style="display:<%#page.totRow==0?"none":""%>" border="0" class="bluetable" cellspacing="1" cellpadding="2" width="100%" align="center" id="dataList">
	    <thead>
            <Tr>
	            ...
	            <td align="center" class="lightbluetable">目前<br>進度</td>
	            ...
	            <td align="center" class="lightbluetable">結案原因</td>
            </tr>
	    </thead>
	    <tbody>
</HeaderTemplate>
			<ItemTemplate>
 		        <tr class="<%#(Container.ItemIndex + 1) % 2 == 1 ? "sfont9" : "lightbluetable3"%>">
	                ...
	                <td align="center">
	                    ...
	                    <input type="hidden" id=scode_<%#(Container.ItemIndex + 1)%> name=scode_<%#(Container.ItemIndex + 1)%> value="<%#Eval("dmt_scode")%>">
	                </td>
                    <%}%>
	                ...
	                <td align="center" style="cursor: pointer;" onmouseover="this.style.color='red'" onmouseout="this.style.color='black'"  onclick="QstepClick('<%#Eval("seq")%>', '<%#Eval("seq1")%>')">
                        <%#Eval("now_step_grade")%>
	                </td>
