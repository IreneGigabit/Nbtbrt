<%@ Control Language="C#" ClassName="opt_case_form" %>
...
<script runat="server">
    ...
    private void Page_Load(System.Object sender, System.EventArgs e) {
        ...
        using (DBHelper conn = new DBHelper(Conn.btbrt).Debug(false)) {
            ...
            F_tscode = Util.Option(conn, "select distinct scode,sc_name,scode1 from sysctrl.dbo.vscode_roles where branch='" + branch + "' and dept='T' and syscode='" + branch + "Tbrt' and roles='sales' order by scode1", "{scode}", "{sc_name}");
            tfy_receipt_title = Sys.getCustCode("rec_titleT", "", "").Option("{cust_code}", "{code_name}");
            tfy_Ar_mark = Sys.getCustCode("ar_mark", "and (mark1 like '%" + Session["SeBranch"] + Session["Dept"] + "%' or mark1 is null)", "").Option("{cust_code}", "{code_name}");
