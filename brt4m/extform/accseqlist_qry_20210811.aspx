...
<script runat="server">
    ...
    DataTable dtAcctPin = new DataTable();//收入1
    DataTable dtOAcctPlus = new DataTable();//收入2
    DataTable dtAcctPlus = new DataTable();//收入3
    DataTable dtOAcctPlus21 = new DataTable();//規費1
    DataTable dtAcctPlus21 = new DataTable();//規費2
    ...
    private void QueryData() {
        ...
        //抓取收入資料1
        int plusRow = 0;
        SQL = "select arcase,ar_no,in_date as ardate,service,fees,tax_out,acc_date,casegrp,in_curr,db_no,''tclass";
        ...
        connacc.DataTable(SQL, dtAcctPin);
        if (dtAcctPin.Rows.Count > 0) {
            for (int i = 0; i < dtAcctPin.Rows.Count; i++) {
                DataRow dr = dtAcctPin.Rows[i];
                //行樣式
                dr["tclass"] = (++plusRow) % 2 == 1 ? "sfont9" : "lightbluetable3";
            }
            acctPinRepeater.DataSource = dtAcctPin;
            acctPinRepeater.DataBind();
            //合計
            sum_pin_service = Convert.ToInt32(dtAcctPin.Compute("Sum(service)", ""));//服務費
            sum_pin_fees = Convert.ToInt32(dtAcctPin.Compute("Sum(fees)", ""));//規費
            sum_pin_tax_out = Convert.ToInt32(dtAcctPin.Compute("Sum(tax_out)", ""));//營業稅
        }

        //抓取收入調整資料-oacct_plus
        SQL = "select arcase,ar_no,plus_date as ardate,nt_money,acc_code,remark,casegrp,tran_no,acc_sqlno,db_no,dc_code,0.00 plus_add,0.00 plus_del,''tclass";
        SQL += ",(select case_name from varcase where dept='" + Session["dept"] + "' and casegrp=a.casegrp and arcase=a.arcase) as case_name ";
        SQL += " from oacct_plus a where seq=" + seq + " and seq1='" + seq1 + "' and dept='T' " + wsql + " and acc_code like '4%' order by plus_date ";
        connacc.DataTable(SQL, dtOAcctPlus);
        if (dtOAcctPlus.Rows.Count > 0) {
            for (int i = 0; i < dtOAcctPlus.Rows.Count; i++) {
                DataRow dr = dtOAcctPlus.Rows[i];
            ...
            oacctPlusRepeater.DataSource = dtOAcctPlus;
            oacctPlusRepeater.DataBind();
            //合計
            sum_plus_add = Convert.ToInt32(dtOAcctPlus.Compute("Sum(plus_add)", ""));//收入加項
            sum_plus_del = Convert.ToInt32(dtOAcctPlus.Compute("Sum(plus_del)", ""));//收入減項
        }
        ...
        connacc.DataTable(SQL, dtAcctPlus);
        if (dtAcctPlus.Rows.Count > 0) {
            for (int i = 0; i < dtAcctPlus.Rows.Count; i++) {
                DataRow dr = dtAcctPlus.Rows[i];
            ...
            acctPlusRepeater.DataSource = dtAcctPlus;
            acctPlusRepeater.DataBind();
            //合計
            sum_plus_add += Convert.ToInt32(dtAcctPlus.Compute("Sum(plus_add)", ""));//收入加項
            sum_plus_del += Convert.ToInt32(dtAcctPlus.Compute("Sum(plus_del)", ""));//收入減項
        }

        //抓取規費資料1-oacct_plus
        int acctRow = 0;
        SQL = "select arcase,ar_no,plus_date as ardate,nt_money,acc_code1,dc_code,plus_no,excase,ex_rate,currency,tr_money,remark,casegrp,tran_no,acc_sqlno ";
        SQL += ",(select case_name from varcase where dept='" + Session["dept"] + "' and casegrp=a.casegrp and arcase=a.arcase) as case_name";
        ...
        connacc.DataTable(SQL, dtOAcctPlus21);
        if (dtOAcctPlus21.Rows.Count > 0) {
            for (int i = 0; i < dtOAcctPlus21.Rows.Count; i++) {
                DataRow dr = dtOAcctPlus21.Rows[i];
            ...
            oacctPlus21Repeater.DataSource = dtOAcctPlus21;
            oacctPlus21Repeater.DataBind();
            //合計
            sum_fees = Convert.ToInt32(dtOAcctPlus21.Compute("Sum(fees)", ""));//應付規費(D)
            sum_fees_add = Convert.ToInt32(dtOAcctPlus21.Compute("Sum(fees_add)", ""));//應付規費(+)
            sum_fees_del = Convert.ToInt32(dtOAcctPlus21.Compute("Sum(fees_del)", ""));//應付規費(-)
        }
        ...
        connacc.DataTable(SQL, dtAcctPlus21);
        if (dtAcctPlus21.Rows.Count > 0) {
            for (int i = 0; i < dtAcctPlus21.Rows.Count; i++) {
                DataRow dr = dtAcctPlus21.Rows[i];
            ...
            acctPlus21Repeater.DataSource = dtAcctPlus21;
            acctPlus21Repeater.DataBind();
            //合計
            sum_fees = Convert.ToInt32(dtAcctPlus21.Compute("Sum(fees)", ""));//應付規費(D)
            sum_fees_add = Convert.ToInt32(dtAcctPlus21.Compute("Sum(fees_add)", ""));//應付規費(+)
            sum_fees_del = Convert.ToInt32(dtAcctPlus21.Compute("Sum(fees_del)", ""));//應付規費(-)
        }
...
<div align="center" id="noData" style="display:<%#dt.Rows.Count==0?"":"none"%>">
    ...
    <br />
    <table style="display:<%#((dtAcctPin.Rows.Count==0&&dtOAcctPlus.Rows.Count==0&&dtAcctPlus.Rows.Count==0)?"none":"")%>" border="0" class="bluetable" cellspacing="1" cellpadding="2" width="100%" align="center">
	    <tr align="center">  
    ...
    <asp:Repeater id="acctPinRepeater" runat="server">
	<ItemTemplate>
 	    <tr class="<%#Eval("tclass")%>">
		    <td align="center">**1**<%#Eval("arcase")%></td>
		    <td align="center"><%#Eval("case_name")%></td>
		    <td align="center"><%#Eval("ar_no")%><%#(Convert.ToInt32(Eval("in_curr"))>1 ?"-"+Eval("in_curr"):"")%></td>
		    <td align="center"><%#Eval("db_no")%></td>
    ...
    <asp:Repeater id="oacctPlusRepeater" runat="server">
	<ItemTemplate>
 	    <tr class="<%#Eval("tclass")%>">
		    <td align="center">**2**<%#Eval("arcase")%></td>
		    <td align="center"><%#Eval("case_name")%></td>
		    <td align="center"><%#Eval("ar_no")%></td>
		    <td align="center"><%#Eval("db_no")%></td>
		    <td align="center"><%#Eval("tran_no")%></td>
		    <td align="center"><%#Eval("ardate","{0:d}")%></td>
		    <td align="center" colspan=4><%#Eval("remark")%></td>
		    <td align="right"><%#Eval("plus_add","{0:0}")%></td> 
		    <td align="right"><%#Eval("plus_del","{0:0}")%></td>
	    </tr>
	</ItemTemplate>
    </asp:Repeater>
    <asp:Repeater id="acctPlusRepeater" runat="server">
	<ItemTemplate>
 	    <tr class="<%#Eval("tclass")%>">
		    <td align="center">**3**<%#Eval("arcase")%></td>
		    <td align="center"><%#Eval("case_name")%></td>
		    <td align="center"><%#Eval("ar_no")%></td>
		    <td align="center"><%#Eval("db_no")%></td>
		    <td align="center"><%#Eval("tran_no")%></td>
    ...
    <br />
    <table style="display:<%#((dtOAcctPlus21.Rows.Count==0&&dtAcctPlus21.Rows.Count==0)?"none":"")%>" border="0" class="bluetable" cellspacing="1" cellpadding="2" width="100%" align="center">
	    <tr align="center">  
    ...
    <asp:Repeater id="oacctPlus21Repeater" runat="server">
	<ItemTemplate>
 	    <tr class="<%#Eval("tclass")%>">
		    <td align="center" rowspan=2><%#Eval("arcase")%></td>
    ...
    <asp:Repeater id="acctPlus21Repeater" runat="server">
	<ItemTemplate>
 	    <tr class="<%#Eval("tclass")%>">
		    <td align="center" rowspan=2><%#Eval("arcase")%></td>
		    <td align="center" rowspan=2><%#Eval("case_name")%></td> 
