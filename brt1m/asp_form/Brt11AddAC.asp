...
'申請人入log_table
	 ...
	 If Trim(Request("chkTest"))<>Empty Then Response.Write "9=" & dSQL7 & "<hr>"
	 cmd.Execute(dSQL7)
	for apnum=1 to trim(request("apnum"))
		...
		IF SQL7<>empty then
			...
			If Trim(Request("chkTest"))<>Empty Then Response.Write "10=" & SQL7 & "<hr>"
			cmd.Execute(SQL7)
		End IF
		'Response.Write "apnum="&SQL7&"<br>"
	next 
...
'	next 
	...
	if trim(request("grconf_sqlno"))<>empty then
	   ...
	   If Trim(Request("chkTest"))<>Empty Then Response.Write "11=" & SQL4 & "<hr>"
	   cmd.Execute(SQL4)
	End if	
...
sql = left(sql,len(sql)-1) & sqlWHERE   
  ...
  cmd.CommandText=SQL
  If Trim(Request("chkTest"))<>Empty Then Response.Write "13=" & SQL & "<hr>"
  cmd.Execute(SQL)
...
'	next 	
	...
	if trim(request("grconf_sqlno"))<>empty then
	   sql4 = "update grconf_dmt set job_no='" & request("in_no") & "',finish_date=getdate() where grconf_sqlno=" & request("grconf_sqlno")
	   cmd.CommandText=SQL4	
	   If Trim(Request("chkTest"))<>Empty Then Response.Write "19=" & SQL4 & "<hr>"
	   cmd.Execute(SQL4)
	End if		
