...
'申請人入log_table
	 ...
	 If Trim(Request("chkTest"))<>Empty Then Response.Write "17=" & dSQL7 & "<hr>"
	 cmd.Execute(dSQL7)
	for apnum=1 to trim(request("apnum"))
		...
		IF SQL7<>empty then
			...
			If Trim(Request("chkTest"))<>Empty Then Response.Write "18=" & SQL7 & "<hr>"
			cmd.Execute(SQL7)
		End IF
		'Response.Write "apnum="&SQL7&"<br>"
	next	
...
End IF		
	...
	if trim(request("grconf_sqlno"))<>empty then
	   ...
	   If Trim(Request("chkTest"))<>Empty Then Response.Write "28=" & SQL4 & "<hr>"
	   cmd.Execute(SQL4)
	End if
	...
	IF trim(Request("tfzr_class_count"))<>empty then
		for x=1 to ctrlnum 
			...
			ELSE
				...
				IF SQL6<>EMPTY THEN
					cmd.CommandText=SQL6
					If Trim(Request("chkTest"))<>Empty Then Response.Write "32=" & SQL6 & "<hr>"
					cmd.Execute(SQL6)
				END IF
		next
	 End IF	
...
'*****移轉檔	
  ...
  cmd.CommandText=SQL
   ...
   dsql="delete from dmt_tranlist where in_no='" & trim(request("in_no")) & "' and in_scode='" & trim(request("in_scode")) & "' and mod_field='mod_ap'"
    cmd.CommandText=dSQL
	If Trim(Request("chkTest"))<>Empty Then Response.Write "34=" & dSQL & "<hr>"
	cmd.Execute(dSQL)
	for k=1 to request("ft_apnum")
		 ocname1 = replace(trim(request("tfr_ocname1_" & k)),"'","’")
		 ocname1 = replace(ocname1,"&","＆")
	...
	'申請人入log_table
	 ...
	 dSQL7="Delete dmt_temp_ap where in_no='"& trim(request("in_no")) &"' and case_sqlno=0"
	 cmd.CommandText=dSQL7
	 If Trim(Request("chkTest"))<>Empty Then Response.Write "36=" & dSQL7 & "<hr>"
	 cmd.Execute(dSQL7)
	for apnum=1 to trim(request("apnum"))
		ap_cname = replace(trim(request("ap_cname" & apnum)),"'","’")
			if len(ap_cname) > 0 then
		...
		IF SQL7<>empty then
			cmd.CommandText=SQL7
			If Trim(Request("chkTest"))<>Empty Then Response.Write "37=" & SQL7 & "<hr>"
			cmd.Execute(SQL7)
		End IF
		'Response.Write "apnum="&SQL7&"<br>"
	next	 
...
'	next 		
	...
	if trim(request("grconf_sqlno"))<>empty then
	   sql4 = "update grconf_dmt set job_no='" & request("in_no") & "',finish_date=getdate() where grconf_sqlno=" & request("grconf_sqlno")
	   cmd.CommandText=SQL4	
	   If Trim(Request("chkTest"))<>Empty Then Response.Write "38=" & SQL4 & "<hr>"
	   cmd.Execute(SQL4)
	End if
  SQL="select max(sqlno) as Msqlno from ToDoList where in_no = '" & tno & "' and in_scode = '" & tscode & "' and apcode='Si04W02' and dowhat='DC'"
   ...
   if not RTreg.EOF then				
		...
		sql=sqlins
		If Trim(Request("chkTest"))<>Empty Then Response.Write "39=" & SQL & "<hr>"
		connsys.execute(sql) 	
  end if
  RTreg.close	 
