...
End If 	
	...
	Elseif	reg.tfy_case_stat.value="OO" then
		...
		If reg.Old_seq.value = empty then
		   msgbox "當案件為舊案舊案時，請輸入『案件編號』及按下『確定』以取得詳細資料!"
		   settab 4
		   reg.Old_seq.focus
		   exit Function
		End if							
...
if left(reg.tfy_arcase.value,3)="FP1" then
	'2012/7/1新申請書取消質權期間，2012/10/10修改
	'If reg.tfg1_term1.value = empty then
	'		msgbox "質權起日不得為空白,請重新輸入"
	'		settab 5
	'		reg.tfg1_term1.focus
	'		exit Function
	'else
	'	IF isdate(reg.tfg1_term1.value) = false then
	'		msgbox "請檢查備註日期 ，日期格式是否正確!!"
	'		settab 5
	'		reg.tfg1_term1.focus
	'		exit function
	...
	'		msgbox "質權迄日不得為空白,請重新輸入"
	'		settab 5
	'		reg.tfg1_term2.focus
	'		exit Function
	'else
	'	IF isdate(reg.tfg1_term2.value) = false then
	'		msgbox "請檢查備註日期 ，日期格式是否正確!!"
	'		settab 5
	'		reg.tfg1_term2.focus
	'		exit function
	'	End If
	'End if
	If reg.tfg1_debit_money.value  = empty then
			msgbox "債權額度不得為空白,請重新輸入"
			settab 5
			reg.tfg1_debit_money.focus
			exit Function
	End if
	'出名代理人檢查
	apclass_flag="N"
...
elseif left(reg.tfy_arcase.value,3)="FP2" then
	If reg.tfg2_term1.value = empty then
			msgbox "質權消滅日期不得為空白,請重新輸入"
			'settab 6
			settab 5	'2015/8/12修改
			reg.tfg2_term1.focus
			exit Function
	else
		IF isdate(reg.tfg2_term1.value) = false then
			msgbox "請檢查質權消滅日期 ，日期格式是否正確!!"
			'settab 6
			settab 5	'2015/8/12修改
			reg.tfg2_term1.focus
			exit function
