...
if trim(reg.tfzd_issue_no.value) = empty then	
    answer=MsgBox("註冊號數為空白，確定交辦？",vbYesNo+vbdefaultbutton2,"確認註冊號數")
	...
	Elseif	reg.tfy_case_stat.value="OO" then
		...
		If reg.Old_seq.value = empty then
		   msgbox "當案件為舊案授權時，請輸入『案件編號』及按下『確定』以取得詳細資料!"
		   settab 4
		   reg.Old_seq.focus
		   exit Function
		End if							
...
IF (left(reg.tfy_arcase.value,3)="FL1" or left(reg.tfy_arcase.value,3)="FL2" or left(reg.tfy_arcase.value,3)="FL5" or left(reg.tfy_arcase.value,3)="FL6") and reg.tfg1_mod_claim1(1).checked=true then
   if reg.fl_term1.value=empty then
	   ...
	   IF isdate(reg.fl_term1.value) = false then
			msgbox "請檢查"&errName&"，日期格式是否正確!!"
			settab 5
			reg.fl_term1.focus
			exit function
