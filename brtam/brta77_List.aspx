...
<script runat="server">
    protected string HTProgCap = "程序轉案完成確認(轉出)";//HttpContext.Current.Request["prgname"];//功能名稱
    ...
    protected string qs_dept = "", dept_nm="";
    ...
    private void PageLayout() {
        ...
        if ((HTProgRight & 4) > 0 || (HTProgRight & 8) > 0 || (HTProgRight & 16) > 0 || (HTProgRight & 64) > 0) {
            StrFormBtn += "<input type=button value ='轉案完成確認' class='cbutton bsubmit' onclick='formSubmit()'>\n";
        ...
        if (qs_dept == "t") {
            ...
            SQL += " where syscode='" + Session["syscode"] + "' and dowhat='TRAN_ED1' and job_status='NN' ";
            SQL += "order by scode1 ";
        } else if (qs_dept == "e") {
            SQL = "select distinct a.scode,(select sc_name from sysctrl.dbo.scode where scode=a.scode) as sc_name ";
            SQL += ",(select sscode from sysctrl.dbo.scode where scode=a.scode) as scode1 ";
    ...
    private void QueryData() {
        ...
        if (ReqVal.TryGet("sdate") != "") {
            wsql += " and c.in_date>='" + ReqVal["sdate"] + " 00:00:00' ";
        }
        if (ReqVal.TryGet("edate") != "") {
            wsql += " and c.in_date<='" + ReqVal["edate"] + " 23:59:59' ";
        }
        ...
        if (qs_dept == "t") {
            SQL = "select a.*,b.appl_name,b.class,b.scode,b.step_grade,b.end_date,b.cust_area,b.cust_seq,b.cust_seq1,'' as country,c.sqlno as todo_sqlno,c.in_date as conf_date ";
            ...
            SQL += ",''fseq,''ftran_seq,'todo_dmt'todo_tblnm ";
            SQL += " from dmt_brtran a ";
            SQL += " inner join dmt b on a.seq = b.seq and a.seq1 = b.seq1 ";
            SQL += " inner join todo_dmt c on a.seq=c.seq and a.seq1=c.seq1 and c.dowhat='TRAN_ED1' ";
        } else if (qs_dept == "e") {
            SQL = "select a.*,b.appl_name,b.class,b.scode,b.step_grade,b.end_date,b.cust_area,b.cust_seq,b.cust_seq1,b.country as country,c.sqlno as todo_sqlno,c.in_date as conf_date ";
            SQL += ",(select cust_name from view_cust where view_cust.cust_area = b.cust_area and view_cust.cust_seq = b.cust_seq) as cust_name ";
            SQL += ",(select sc_name from sysctrl.dbo.scode where scode = b.scode) as scode_name ";
            SQL += ",(select code_name from cust_code where code_type='Tecasestat' and cust_code=b.now_stat) as now_stat_name  ";
            SQL += ",(select branchname from sysctrl.dbo.branch_code where branch=a.tran_seq_branch) as branchname ";
            SQL += ",''fseq,''ftran_seq,'todo_ext'todo_tblnm ";
            SQL += " from ext_brtran a ";
            SQL += " inner join ext b on a.seq = b.seq and a.seq1 = b.seq1 ";
            SQL += " inner join todo_ext c on a.seq=c.seq and a.seq1=c.seq1 and c.dowhat='TRAN_ED1' ";
        }
        ...
        for (int i = 0; i < page.pagedTable.Rows.Count; i++) {
            ...
            //新案號
            dr["ftran_seq"] = Sys.formatSeq(dr.SafeRead("tran_seq", ""), dr.SafeRead("tran_seq1", ""), dr.SafeRead("country", ""), dr.SafeRead("tran_seq_branch", ""), dept_nm);
        }
...
<form style="margin:0;" id="reg" name="reg" method="post">
	...
	<input type="hidden" id=rows_tran_seq name=rows_tran_seq />
	<input type="hidden" id=rows_tran_seq1 name=rows_tran_seq1 />
        ...
        <table style="display:<%#page.totRow==0?"none":""%>" border="0" class="bluetable" cellspacing="1" cellpadding="2" width="100%" align="center" id="dataList">
	        <thead>
                  <Tr class="lightbluetable" align="center">
		            ...
		            <td>結案日期</td>
		            <td>案件狀態</td>
		            <td><u class="setOdr" v1="a.sc_date">營洽轉案處理日</u></td>
		            <td>轉案原因</td>
		            <td>新單位案件編號</td>
		            <td><u class="setOdr" v1="c.in_date">新單位通知轉案完成日</u></td>
	            </tr>
	        </thead>
	        <tbody>
    </HeaderTemplate>
		<ItemTemplate>
 	        <tr class="<%#(Container.ItemIndex+1)%2== 1 ?"sfont9":"lightbluetable3"%>">
		        <td align=center>
		            ...
		            <input type="hidden" id=tran_seq_<%#(Container.ItemIndex+1)%> value="<%#Eval("tran_seq")%>">
		            <input type="hidden" id=tran_seq1_<%#(Container.ItemIndex+1)%> value="<%#Eval("tran_seq1")%>">
		        ...
		        <td align="center"><%#Eval("ftran_seq")%></td>
		        <td align="center"><%#Eval("conf_date")%></td>
	        </tr>
		</ItemTemplate>
    <FooterTemplate>
	        </tbody>
        ...
        <table border="0" width="100%" cellspacing="0" cellpadding="0">
		    <tr class="FormName"><td>
			    <div align="left" style="color:blue">
			        ※流程:原單位營洽轉案處理-->原單位組主管簽核-->原單位部門主管簽核-->原單位區所主管簽核-->原單位程序轉案發文確認-->新單位主管確認轉案-->新單位程序確認轉案--><font color=red>原單位程序轉案完成確認</font>
			    </div>
		    </td>
            </tr>
	    </table>
...
<script language="javascript" type="text/javascript">
    ...
    //確認
    function formSubmit(){
        $("#rows_chkflag,#rows_brtran_sqlno,#rows_todo_sqlno,#rows_seq,#rows_seq1,#rows_appl_name,#rows_scode,#rows_cust_seq,#rows_cust_seq1,#rows_cust_name,#rows_tran_seq_branch,#rows_tran_seq,#rows_tran_seq1,#rows_tran_remark").val("");

        var totnum=$("input[id^='chkflag_']:checked").length;
        if (totnum==0){
            alert("請勾選您要轉案完成確認的案件!!");
            return false;
        }else{
            var tans = confirm("共有" + totnum + "筆需要轉案完成確認 , 是否確定?");
            ...
            $("#rows_tran_seq").val(getJoinValue("#dataList>tbody input[id^='tran_seq_']:not([id^='tran_seq_branch_'])"));
            $("#rows_tran_seq1").val(getJoinValue("#dataList>tbody input[id^='tran_seq1_']"));
            ...
            ajaxByForm("Brta77_Update.aspx",formData)
