    ...
    private void QueryData() {
        ...
        if (prgid1 != "brta81") {//正常的官發作業
            if (submitTask == "A") {
                ...
                step_data["fees"] = "0";
                step_data["fees_stat"] = "N";
                step_data["step_date"] = DateTime.Today.ToShortDateString();

                //總收發文日期
                //台北所總收發當天就會發文
                step_data["mp_date"] = DateTime.Today.ToShortDateString();
                if (Sys.GetSession("seBranch") != "N") {
                    switch (DateTime.Today.DayOfWeek) {
                        case DayOfWeek.Friday: step_data["mp_date"] = DateTime.Today.AddDays(3).ToShortDateString(); break;//星期五加三天
                        case DayOfWeek.Saturday: step_data["mp_date"] = DateTime.Today.AddDays(2).ToShortDateString(); break;//星期六加兩天
                        default: step_data["mp_date"] = DateTime.Today.AddDays(1).ToShortDateString(); break;//加一天
                    }
                }

                step_data["rs_type"] = Sys.getRsType();
                step_data["opt_branch"] = Sys.GetSession("seBranch");
            }
            ...
            if (submitTask == "U" || submitTask == "Q" || submitTask == "D") {
                ...
                if (dtStep.Rows.Count > 0) {
                    ...
                    step_data["old_send_way"] = (objResult == DBNull.Value || objResult == null) ? "" : objResult.ToString();
                    ...
                    SQL = "select rs_detail from code_br where rs_type='" + step_data["rs_type"] + "' and rs_code='" + step_data["rs_code"] + "' and gs='Y' ";
                    objResult = conn.ExecuteScalar(SQL);
                    ...
                    step_data["fees"] = dr.SafeRead("fees", "");
                    ...
                    //取得案性mark
                    SQL = "select mark from code_br where dept='T' and rs_type='" + step_data["rs_type"] + "' and rs_class='" + step_data["rs_class"] + "' and rs_code='" + step_data["rs_code"] + "' and gs='Y'";
                    objResult = conn.ExecuteScalar(SQL);
                    step_data["markb"] = (objResult == DBNull.Value || objResult == null) ? "" : objResult.ToString();

                    //收據抬頭
                    step_data["receipt_type"] = dr.SafeRead("receipt_type", "");
                    step_data["receipt_title"] = dr.SafeRead("receipt_title", "");
                    step_data["rectitle_name"] = dr.SafeRead("rectitle_name", "");
                    //若為電子送件,預設收據種類為電子收據
                    if (step_data["receipt_type"] == "") {
                        if (step_data["send_way"] == "E")
                            step_data["receipt_type"] = "E";
                        else
                            step_data["receipt_type"] = "P";
                    }
                    //若為電子送件,設定預設值
                    if (step_data["receipt_title"] == "") {
                        if (step_data["send_way"] == "E") {
                            step_data["receipt_title"] = Sys.getDefaultTitle();
                        } else {
                            step_data["receipt_title"] = "B";
                        }
                    }

                    //檢查array.account.plus_temp.chk_type='Y'表會計已確認，只要有一筆 ="Y"就要有警語
                    step_data["chk_typestr"] = "";
                    using (DBHelper conni = new DBHelper(Conn.account).Debug(Request["chkTest"] == "TEST")) {
                        ...
                        SQL += " and rs_no='" + step_data["rs_no"] + "' and chk_type='Y'";
                        using (SqlDataReader dr0 = conni.ExecuteReader(SQL)) {
                            if (dr0.HasRows) {
                                while (dr0.Read()) {
                                    step_data["chk_typestr"] += "," + dr0.SafeRead("case_no", "");
                                }
                                step_data["chk_typestr"] = "(會計已確認:" + step_data["chk_typestr"].Substring(1) + ")";
                            }
                        }
                    }
                }
            ...
            if (dtStep.Rows.Count > 0) {
                ...
                step_data["fseq"] = Sys.formatSeq(step_data["seq"], step_data["seq1"], "", Sys.GetSession("seBranch"), Sys.GetSession("dept"));
                step_data["step_date"] = dr.GetDateTimeString("step_date", "yyyy/M/d");
                step_data["Send_dept"] = dr.SafeRead("Send_dept", "");
                step_data["mp_date"] = dr.GetDateTimeString("mp_date", "yyyy/M/d");
                step_data["send_cl"] = dr.SafeRead("send_cl", "");
                step_data["send_cl1"] = dr.SafeRead("send_cl1", "");
                step_data["send_sel"] = dr.SafeRead("send_sel", "");
                step_data["opt_sqlno"] = dr.SafeRead("opt_sqlno", "");
                step_data["rs_type"] = Sys.getRsType();
                step_data["rs_class"] = dr.SafeRead("rs_class", "");
                step_data["rs_code"] = dr.SafeRead("rs_code", "");
                step_data["act_code"] = dr.SafeRead("act_code", "");
