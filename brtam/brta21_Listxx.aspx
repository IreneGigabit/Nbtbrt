...
<script runat="server">
    ...
    private void QueryData() {
        if (cgrs == "CS") {
            ...
            SQL += ",*,''fseq,''step_grade,rs_no main_rs_no,''urlasp,''a_last_date,''scode1_name,''lcg,''lrs ";
            SQL += " from vcs_dmt ";
            SQL += " where 1=1";
        } else {
    ...
    protected string GetScode1Name(DataRow dr) {
        string scode1_name = "";
        if (cgrs == "CS") {
            scode1_name = dr.SafeRead("sc_name", "");
        } else {
    ...
    protected string GetLink(RepeaterItem Container) {
        ...
        if (cgrs == "GS") {
            ...
            using (SqlDataReader dr = connm.ExecuteReader(SQL)) {
                if (dr.Read()) {
                    if (dr.SafeRead("chk_type", "") == "NN" || dr.SafeRead("chk_type", "") == "NX") {
                        send_status = "N";
                    } else {
                        send_status = "Y";
                    }
        ...
        //[維護]
        string linkU = "<a href=\"" + editaspx + "?submitTask=U&prgid=" + prgid + "&main_rs_no=" + Eval("main_rs_no") + "&rs_no=" + Eval("rs_no") + "&aseq=" + Eval("seq") + "&aseq1=" + Eval("seq1") + "&FrameBlank=50\" target=\"Eblank\">";
        if ((HTProgRight & 8) != 0) {
            if (cgrs == "GS") {
                if (send_status != "Y") {
                    link += linkU + "[維護]</a>";
        ...
        //[刪除]
        string linkD = "<a href=\"" + editaspx + "?submitTask=D&prgid=" + prgid + "&main_rs_no=" + Eval("main_rs_no") + "&rs_no=" + Eval("rs_no") + "&aseq=" + Eval("seq") + "&aseq1=" + Eval("seq1") + "&FrameBlank=50\" target=\"Eblank\">";
        ...
        //[查詢]
        link += "<a href=\"" + editaspx + "?submitTask=Q&prgid=" + prgid + "&main_rs_no=" + Eval("main_rs_no") + "&rs_no=" + Eval("rs_no") + "&aseq=" + Eval("seq") + "&aseq1=" + Eval("seq1") + "&FrameBlank=50\" target=\"Eblank\">[查詢]</a>";

        //[列印]
        string step_date = Eval("last_date", "{0:yyyy/M/d}");
        if (cgrs == "GS") {
        ...
        <table style="display:<%#page.totRow==0?"none":""%>" border="0" class="bluetable" cellspacing="1" cellpadding="2" width="100%" align="center" id="dataList">
	        <thead>
                  <Tr class="lightbluetable">
		            ...
		            <td><span class=rsnotitle>收/發文</span>內容</td>
