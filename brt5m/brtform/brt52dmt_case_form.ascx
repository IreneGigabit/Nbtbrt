<%@ Control Language="C#" ClassName="brt52dmt_case_form" %>
        ...
        using (DBHelper conn = new DBHelper(Conn.btbrt).Debug(false)) {
            ...
            using (DBHelper cnn = new DBHelper(Conn.Sysctrl).Debug(false)) {
                ...
                if ((HTProgRight & 64) != 0) {
                    ...
                    td_tscode = "<select id='F_tscode' name='F_tscode' class='" + Lock["ar_curr"] + "'>" + dt.Option("{scode}", "{sc_name}") + "</select>";
                } else {
...
<TR id="tr_fees">
	<TD class=lightbluetable align=left colspan=6><strong>案性及費用：</strong>
        <input type="button" class="cbutton <%=Lock.TryGet("brt51")%> <%=Lock.TryGet("brt52")%>" value="增加請款項目" onclick="case_form.ta_display('Add')">
        <input type="button" class="cbutton <%=Lock.TryGet("brt51")%> <%=Lock.TryGet("brt52")%>" value="減少請款項目" onclick="case_form.ta_display('Del')">
	</TD>
</TR>
<TR>
	    ...
	    <TABLE border=0 class=bluetable cellspacing=1 cellpadding=2 width="100%">
		    <TR>
		        ...
		        <TD class=whitetablebg width=10%>
                    <select id=tfy_Arcase NAME=tfy_Arcase onchange="case_form.toArcase('T',this.value ,'Z1')" disabled><%#tfy_Arcase%></SELECT>
		        </TD>
		        <TD class=lightbluetable align=right width=3%>服務費：</TD>
		        <TD class=whitetablebg  align="left">
                    <INPUT TYPE=text id=nfyi_Service name=nfyi_Service value=0 SIZE=8 maxlength=8 style="text-align:right;" disabled>
                    <INPUT TYPE=hidden id=Service name=Service>
		        </TD>
		        <TD class=lightbluetable align=right width=3%>規費：</TD>
		        <TD class=whitetablebg align="left">
                    <INPUT TYPE=text id=nfyi_Fees name=nfyi_Fees value=0 SIZE=8 maxlength=8 style="text-align:right;" onblur="case_form.summary()" disabled>
            ...
            <script type="text/html" id="ta_template"><!--其他費用樣板-->
	            <tr id=tr_ta_##>
		            ...
		            <td class=whitetablebg align=left width="10%">
		                <select id="nfyi_item_Arcase_##" name="nfyi_item_Arcase_##" onchange="case_form.ToFee('T',this.value ,reg.ar_form.value,'##')" disabled>
                        <%#nfyi_item_Arcase%>
		                </select> x <input type=text id="nfyi_item_count_##" name="nfyi_item_count_##" size=3 maxlength=3 value="1" onblur="case_form.item_count('##')" disabled>項
		            </td>
		            <td class=lightbluetable align=right width=4%>服務費：</td>
		            <td class=whitetablebg align=left width=5%>
		                <INPUT TYPE=text id=nfyi_Service_## name=nfyi_Service_## SIZE=8 maxlength=8 style="text-align:right;" value="0" onblur="case_form.summary()" disabled>
		                <input type=text id=nfzi_Service_## name=nfzi_Service_##>
		            </td>
		            <td class=lightbluetable align=right width=4%>規費：</td>
		            <td class=whitetablebg align=left width=5%>
		                <INPUT TYPE=text id=nfyi_fees_## name=nfyi_fees_## SIZE=8 maxlength=8 style="text-align:right;" value="0" onblur="case_form.item_nfyi_fees('##')" disabled>
		    ...
		    <TR>
			    ...
			    <TD class=whitetablebg width="11%">
                    <select id=tfy_oth_arcase NAME=tfy_oth_arcase onchange="case_form.ToFee('T',this.value ,reg.ar_form.value,'10')" disabled><%#tfy_oth_arcase%></SELECT>
			    </TD>
			    <TD class=lightbluetable align=right width=4%>轉帳金額：</TD>
			    <TD class=whitetablebg width=5%><input type="text" id="nfy_oth_money" name="nfy_oth_money" size="8" style="text-align:right;" onblur="case_form.summary()" disabled></TD>
			    <TD class=lightbluetable align=right width=4%>轉帳單位：</TD>
			    <TD class=whitetablebg width=5%>
			        <select id=tfy_oth_code NAME=tfy_oth_code disabled>
...
<TR>
	...
	<TD class=whitetablebg>
        <Select id=tfy_Ar_mark name=tfy_Ar_mark onchange="case_form.special()" disabled><%#tfy_Ar_mark%></Select>
	</TD>
	...
	<TD class="whitetablebg" colspan="3">
	    ...
	    <INPUT TYPE=checkbox id=tfy_discount_chk name=tfy_discount_chk value="Y" disabled>折扣請核單
...
<TR>
	...
	<TD class=whitetablebg>
        ...
        <Select id=tfy_source name=tfy_source class="<%=Lock.TryGet("ar_curr")%>"><%#tfy_source%></Select>
	</TD>
	...
	<TD class=whitetablebg colspan="3">
        ...
        <input type="radio" id="Contract_no_Type_N" name="Contract_no_Type" value="N" class="<%=Lock.TryGet("ar_curr")%>">
        <INPUT TYPE=text id=tfy_Contract_no name=tfy_Contract_no SIZE=10 MAXLENGTH=10 class="<%=Lock.TryGet("ar_curr")%>" onchange="$('#Contract_no_Type_N').prop('checked',true).trigger('click');">
        <span id="contract_type">
		    <input type="radio" id="Contract_no_Type_A" name="Contract_no_Type" value="A" class="<%=Lock.TryGet("ar_curr")%>">後續案無契約書
		</span>
		<span style="display:none"><!--2015/12/29修改，併入C不顯示-->
		    <input type="radio" id="Contract_no_Type_S" name="Contract_no_Type" value="B" class="<%=Lock.TryGet("ar_curr")%>">特案簽報
		</span>
	    <input type="radio" id="Contract_no_Type_C" name="Contract_no_Type" value="C" class="<%=Lock.TryGet("ar_curr")%>">其他契約書無編號/特案簽報
	    <input type="radio" id="Contract_no_Type_M" name="Contract_no_Type" value="M" class="<%=Lock.TryGet("ar_curr")%>">總契約書
	    <span id="span_btn_contract" style="display:none">
		    ...
		    <input type=button class="greenbutton <%=Lock.TryGet("ar_curr")%>" id="btn_contract" name="btn_contract" value="查詢總契約書">
		    +客戶案件委辦書
	    </span>
	    <br>
		<INPUT TYPE=checkbox id=tfy_contract_flag NAME=tfy_contract_flag value="Y" disabled>契約書相關文件後補，尚缺文件說明：
...
<TR>
    ...
    <TD class=whitetablebg align=left><INPUT type=text id=dfy_last_date NAME=dfy_last_date SIZE=10 class="dateField Lock"></TD>

</TR>
<TR id=tr_send_way>
	<TD class=lightbluetable align=right>發文方式：</TD>
	<TD class=whitetablebg>
        <SELECT id="tfy_send_way" name="tfy_send_way" onchange="case_form.setReceiptType()" disabled><%#tfy_send_way%></select>
        <input type="hidden" id="spe_ctrl" name="spe_ctrl">
	</TD>
	<TD class=lightbluetable align=right>官發收據種類：</TD>
	<TD class=whitetablebg>
		<select id="tfy_receipt_type" name="tfy_receipt_type" onchange="case_form.setReceiptTitle()" disabled>
	...
	<TD class=whitetablebg>
		<select id="tfy_receipt_title" name="tfy_receipt_title" disabled><%#tfy_receipt_title%></select>
...
<script language="javascript" type="text/javascript">
    ...
    case_form.init = function () {
        ...
        if (main.prgid == "brt52") {
            if (CInt(jMain.case_main[0].ar_curr) <= 0 && (jMain.case_main[0].ar_code == "N" || jMain.case_main[0].ar_code == "X" || jMain.case_main[0].ar_code == "O")) {
                $(".td_chkar_code").show();
                if (jMain.case_main[0].ar_code == "O") $("#span_ar_code_O").show();
                if (jMain.case_main[0].ar_code == "N" || jMain.case_main[0].ar_code == "X") $("#span_ar_code_X").show();
            }
        }
    }
