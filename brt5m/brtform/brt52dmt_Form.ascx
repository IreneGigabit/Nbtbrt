<%@ Control Language="C#" ClassName="brt52dmt_form" %>
...
<TABLE border=0 class=bluetable cellspacing=1 cellpadding=2 width="100%">
	<tr>
		...
		<td class=whitetablebg colspan=3>
			<INPUT TYPE=text NAME=tfzb_seq id=tfzb_seq SIZE=<%=Sys.DmtSeq%> MAXLENGTH=<%=Sys.DmtSeq%> class=SEdit readonly>
            -
            <INPUT TYPE=text NAME=tfzb_seq1 id=tfzb_seq1 SIZE=<%=Sys.DmtSeq1%> MAXLENGTH=<%=Sys.DmtSeq1%> style="text-transform:uppercase;" class=SEdit readonly>	
			...
			<input type="hidden" name=tfy_case_stat id=tfy_case_stat>
			<input type="hidden" name=old_seq id=old_seq><!--舊案-->
			<input type="hidden" name=old_seq1 id=old_seq1>
			<input type="hidden" name=New_seq id=New_seq><!--新案-->
			<input type="hidden" name=New_seq1 id=New_seq1>
			<input type="hidden" name=New_Ass_seq id=New_Ass_seq><!--新案(指定編號)-->
			<input type="hidden" name=New_Ass_seq1 id=New_Ass_seq1>
			<input type="hidden" name=btnseq_ok id=btnseq_ok>
			<input type="hidden" name=keyseq id=keyseq>
		</td>
	...
	<tr>
		...
		<td class="whitetablebg" colspan="3">
            <select NAME="tfzy_end_code" id="tfzy_end_code" disabled><%#tfzy_end_code%></select>
...
<script language="javascript" type="text/javascript">
    ...
    dmt_form.init = function () {
        ...
        $("#tfzr_class_count").val(1).triggerHandler("change");//預設顯示第1筆
    }
    ...
    dmt_form.DelAttach_photo = function (pfld) {
        ...
        if (confirm("確定刪除上傳圖檔？")) {
            $.ajax({
                url: getRootPath() + "/sub/del_draw_file_new.aspx?type=dmt_photo&folder_name=&draw_file=" + $("#draw_attach_file").val() + "&btnname=butUpload" + pfld,
                type: 'GET',
                dataType: "script",
                async: false,
                cache: false,
            ...
            //var url = getRootPath() + "/sub/del_draw_file_new.aspx?type=dmt_photo&folder_name=&draw_file=" + $("#draw_attach_file").val() + "&btnname=butUpload" + pfld;
