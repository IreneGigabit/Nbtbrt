...
<script runat="server">
	...
	protected void WordOut() {
		string docFileName = "[顏色團體商標註冊]-" + ipoRpt.Seq + ".docx";
		
		Dictionary<string, string> _tplFile = new Dictionary<string, string>(){
			{"apply", Server.MapPath("~/ReportTemplate/申請書/106顏色團體商標註冊申請書FE6.docx")},
		...
		if (dmt.Rows.Count > 0) {
			...
			switch (dmt.Rows[0]["remark3"].ToString().Trim()) {
				case "商品":
					ipoRpt.ReplaceBookmark("draw_mark3", "虛線部分之商品屬於商標之一部分");
					break;
				case "包裝":
					ipoRpt.ReplaceBookmark("draw_mark3", "虛線部分之包裝屬於商標之一部分");
					break;
				case "容器":
					ipoRpt.ReplaceBookmark("draw_mark3", "虛線部分之容器形狀屬於商標之一部分");
					break;
				case "營業物":
					ipoRpt.ReplaceBookmark("draw_mark3", "虛線部分之營業相關物品形狀屬於商標之一部分");
					break;
				default:
