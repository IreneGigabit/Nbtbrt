...
<script runat="server">
	...
	protected void WordOut() {
		string docFileName = "[證明標章註冊]-" + ipoRpt.Seq + ".docx";

		Dictionary<string, string> _tplFile = new Dictionary<string, string>(){
			{"apply", Server.MapPath("~/ReportTemplate/申請書/201證明標章註冊申請書FED.docx")},
		...
		if (dmt.Rows.Count > 0) {
			...
			//標章圖樣分析
			string cappl_name = dmt.Rows[0]["cappl_name"].ToString().Trim();
			string nzname = dmt.Rows[0]["nzname"].ToString().Trim();
			string eappl_name = dmt.Rows[0]["eappl_name"].ToString().Trim();
			string eappl_name1 = dmt.Rows[0]["eappl_name1"].ToString().Trim();
			string draw = dmt.Rows[0]["draw"].ToString().Trim();
			string symbol = dmt.Rows[0]["symbol"].ToString().Trim();
			if ((cappl_name + nzname + eappl_name + eappl_name1 + draw + symbol) != "") {
				ipoRpt.CopyBlock("b_draw_anal");
				ipoRpt.ReplaceBookmark("ftype", "標章");
				ipoRpt.ReplaceBookmark("cappl_name", cappl_name.ToXmlUnicode(), true);
				ipoRpt.ReplaceBookmark("eappl", (eappl_name + eappl_name1 + nzname) == "" ? "" : "外文", true);
				ipoRpt.ReplaceBookmark("nzname", nzname, true);
				ipoRpt.ReplaceBookmark("eappl_name", eappl_name, true);
				ipoRpt.ReplaceBookmark("eappl_name1", eappl_name1, true);
				ipoRpt.ReplaceBookmark("draw", draw.ToXmlUnicode(), true);
				ipoRpt.ReplaceBookmark("symbol", symbol.ToXmlUnicode(), true);
			}
			//主張優先權
			...
			string pul_name="";
			if (dmt.Rows[0]["pul"].ToString().Trim()=="3"){
				pul_name = "商品";
			}else if(dmt.Rows[0]["pul"].ToString().Trim()=="4"){
				pul_name="服務";
			}
			ipoRpt.ReplaceBookmark("pul", pul_name);
