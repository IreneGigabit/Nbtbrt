...
<script runat="server">
	...
	protected void WordOut() {
		string docFileName = "[其他商標註冊]-" + ipoRpt.Seq + ".docx";

		Dictionary<string, string> _tplFile = new Dictionary<string, string>(){
			{"apply", Server.MapPath("~/ReportTemplate/申請書/111其他商標註冊申請書FEK.docx")},
		...
		if (dmt.Rows.Count > 0) {
			...
			//商標樣態
			if (dmt.Rows[0]["s_mark2"].ToString().Trim() == "H") {
				ipoRpt.ReplaceBookmark("s_mark2", "位置");
			} else if (dmt.Rows[0]["s_mark2"].ToString().Trim() == "I") {
				ipoRpt.ReplaceBookmark("s_mark2", "氣味");
			} else if (dmt.Rows[0]["s_mark2"].ToString().Trim() == "J") {
				ipoRpt.ReplaceBookmark("s_mark2", "觸覺");
			} else {
				ipoRpt.ReplaceBookmark("s_mark2", "");
			}
			//商標圖樣
