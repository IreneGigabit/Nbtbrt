...
<script runat="server">
	...
	protected void WordOut() {
        string docFileName = "[授權登記]-" + ipoRpt.Seq + ".docx";

		Dictionary<string, string> _tplFile = new Dictionary<string, string>(){
			{"apply", Server.MapPath("~/ReportTemplate/申請書/401授權登記申請書FL1.docx")},
		...
		if (dmt.Rows.Count > 0) {
            ...
            using (DataTable dtTran = ipoRpt.Tran) {
                if (dtTran.Rows.Count > 0) {
                    ...
                    if (dtTran.Rows[0]["mod_claim2"].ToString().Trim() == "A") {
                        ipoRpt.ReplaceBookmark("mod_claim2_type", "專屬授權");
                    } else if (dtTran.Rows[0]["mod_claim2"].ToString().Trim() == "B") {
                        ipoRpt.ReplaceBookmark("mod_claim2_type", "非專屬授權");
                    }
                    //授權區域
            ...
            mapList.Add(new AttachMapping { brColValue = remark1, mapValue = "Z1", docType = "07" });//授權契約書
            mapList.Add(new AttachMapping { brColValue = remark1, mapValue = "Z1C", docType = "071" });//授權契約書中文譯本
